	...
	try
	{
		...
		if (!empty($img_all->images))
		{
			...
			for ($i = 0 ; $i < count($img_ary); $i++)
			{
                ...
                $image_thumb = "<img src=\"".$ph_img_all->photo_filename."\" class=\"image_list_thumb_image\" onclick=\"showPopModal('".$ph_img_all->photo_filename."',".$ph_img_all->photo_id.")\"/>";
                print "	<td class=\"point\">".$image_thumb."</td>\r\n";
...
function showPopModal(img_url,photo_id)
{
    setCookie("photo_id", photo_id);
    document.getElementById("hidden_photo_id").value=photo_id;
    document.getElementById("popModalImage").src = img_url;
...
{
    removeCookie("photo_id");
    document.getElementById("hidden_photo_id").value="";
    document.getElementById("popModalImage").src ="";
...
<body>
<div id="zentai" style="min-height: 500px;">
	<!-- メインコンテンツ　ここから -->
	<div id="contents">
    ...
    <div id="mengban" class="mengban_zhezhao"></div>
    <div id="popUpModal" class="divPopModal">
        <div class="divPopModalCenter">
            <img id="popModalImage" src="" class="divPopModalImageThumb"/>
        </div>
        <div class="divPopModalCenter">
            <form enctype="multipart/form-data" method="post" action="./change_thumb_all.php" method="post">
                <input type="hidden" name="hidden_photo_id" id="hidden_photo_id" value=""/>
                <input type="hidden" name="hidden_back_url" value="<?php echo getCurrentPageUrl();?>"/>
                <input type="file" name="p_photo_filename" id="p_photo_filename" value=""  />
                <input type="button" value="×" onclick="clearInputFile()" class="remove_file_btn">
                <input type="submit" value="アップロード" style="font-size: 12px;"/>
            </form>
        </div>
        <div class="divPopModalCenter">
            <input type="button" value="許可する" id="popPermitPhoto" class="image_list_button_permit" onclick="$('#permitPhoto').click();">
            <input type="button" value="閉じる" id="popClosePopupModal" class="close_popup_modal_btn" onclick="hidePopModal();">
        </div>
    </div>
</div>
</body>
