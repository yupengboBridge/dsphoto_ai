...
create_folders(dirname(__FILE__).'/log/error/');
...
if(!defined('DEBUG')){
	...
	function shutdownHandler()
	{
		...
		if($lasterror){
			 ...
			 file_put_contents(dirname(__FILE__).'/log/error/'.date("YmdH").'shutdownlog.txt',$error,FILE_APPEND);
...
if(!defined('DEBUG')){
    ...
    function errorHandler($errno, $errstr = '', $errfile = '', $errline = 0)
	{
		...
		file_put_contents(dirname(__FILE__).'/log/error/'.date("Ymd").'error.txt',$msg,FILE_APPEND);
		switch ($errno)
		{
	...
	function exceptionHandler($ex)
	{
		...
		file_put_contents(dirname(__FILE__).'/log/error/'.date("Ymd").'exception.txt',$msg,FILE_APPEND);
	}
