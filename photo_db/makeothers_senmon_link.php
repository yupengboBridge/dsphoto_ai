...
/*発地の対応表ファイル*/
$HatsuListI = $xRootPath . 'csv/i_p_hatsu_url_v2.csv';
$HatsuListD = $xRootPath . 'csv/d_p_hatsu_url_v2.csv';
...
class MakeKyotenHtmlI{
	...
	function __construct(){
		...
		$dataVarAry = array('p_hatsu','p_hatsu_name','url','letter','view_name','view_type', 'sortnum');
		//対応表つくる
...
class MakeKyotenHtmlD extends MakeKyotenHtmlI{
	...
	function __construct(){
		...
		//データの変数名（海外と国内で変わるから、ここに置いておく）
		$dataVarAry = array('view_type','p_hatsu_name','p_hatsu_sub_old','p_hatsu_sub','p_hatsu_sub_name_old','p_hatsu_sub_name','url','letter','view_name','p_hatsu','sortnum','pull_flg');
		//対応表つくる
		$this->MakeCorHatsuKyoten($dataVarAry);
		/*最適化ファイルで存在する国名を拠点毎にまとめる*/
		//元となるリスト
