...
$max_size_d_p_hatsu_url = 8;

//MakeXml_d_p_hatsu_url("d_p_hatsu_url.csv");
//$flag = MakeXml_d_p_hatsu_url("d_p_hatsu_url.csv");
//if($flag)
//{
//	print "ＸＭＬを出力しました！\r\n";
//}
...
/*
 * 関数名：MakeXml_d_p_hatsu_url
 * 関数説明：ｘｍｌを出力する（出発地URL カレンダー国内用　 検索国内共有）
 * パラメタ：filename　CSVファイル名
 * 戻り値：無し　
 */
function MakeXml_d_p_hatsu_url($filename)
{
	...
	LoadJyogaiCSV_d_p_hatsu_url($filename01);
	...
	while(! feof($file))
	{
		...
		if($row != 1 && $cnt > 0)
		{
			...
			{
				$exists_flg = CheckExists_d_p_hatsu_url($csvarr);
				if($exists_flg)
				{
					continue;
				}
				$hatsuname = mb_convert_kana($csvarr[1],"KVrn","utf-8");	//⇒  mb_convert_kana K :「半角(ﾊﾝｶｸ)片仮名」を「全角片仮名」に変換
				$hatsuname_sub = mb_convert_kana($csvarr[3],"KVrn","utf-8");	//⇒  mb_convert_kana K :「半角(ﾊﾝｶｸ)片仮名」を「全角片仮名」に変換
				$p_hatsuname = mb_convert_kana($csvarr[6],"KVrn","utf-8");	//⇒  mb_convert_kana K :「半角(ﾊﾝｶｸ)片仮名」を「全角片仮名」に変換
...
/*
 * 関数名：CheckExists_d_p_hatsu_url
 * 関数説明：除外リストにあるかどうかチェックする
 * パラメタ：src_line　読み込みのCSVデータ
 * 戻り値：true/false　
 */
function CheckExists_d_p_hatsu_url($src_line)
{
...
/*
 * 関数名：LoadJyogaiCSV_d_p_hatsu_url
 * 関数説明：除外CSVを読み込む
 * パラメタ：filename　CSVファイル名
 * 戻り値：無し　
 */
function LoadJyogaiCSV_d_p_hatsu_url($filename)
{
