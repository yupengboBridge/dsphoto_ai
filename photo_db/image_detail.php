...
$kategori_cnt = 0;//カテゴリカウン//yupengbo add 2011/12/19
try
{
	// ＤＢへ接続します。
	$db_link = db_connect();
...
<link rel="stylesheet" href="./css/skin.css" type="text/css" media="all"/>
    <style type="text/css">
        ...
        .is_publish_span input[type=radio]{
            margin-left: 10px;
        }
        .is_publish_span input[type=button]{
            margin-left: 15px;
            width: 50px;
        }
    </style>
...
function init()
{
	...
	if(obj)
	{
		...
		if(kategori_cnt > 20)
		{
			...
			set_frameheight('iframe_bottom',600);
		}
	}
	//yupengbo modify 2011/12/19 end
}
...
function funcChangePhotoPublish(){
    var photo_id = document.getElementsByName("hidden_photo_id")[0].value;
    var login_id = document.getElementsByName("hidden_login_id")[0].value;

    if(photo_id.length>0){
        var is_publishs = document.getElementsByName("is_publish");
        var is_publish = 0;
        for (var i = 0; i<is_publishs.length; i++) {
            if (is_publishs[i].checked) {
                is_publish = is_publishs[i].value;
                break;
            }
        }
        var url = "change_publish_photo.php?photo_id="+photo_id+"&is_publish="+is_publish+"&login_id="+login_id;
        var res = "";
        $.ajax(
            {
                async:false,
...
function disp_one_image()
{
	...
	{
        print "         <form enctype=\"multipart/form-data\" name=\"register_image_edit\" action=\"./change_thumb_all.php\" method=\"post\">\r\n";
        ...
        print "			<p class='correct_delet_bt'>\r\n";
        print "			    <span class='is_publish_span'>\r\n";
        print "			        <label>公開状況</label>\r\n";

        if($is->images[0]->is_publish==1){
            print "			        <input type='radio' name='is_publish' value='1' checked>公開\r\n";
        }else{
            print "			        <input type='radio' name='is_publish' value='1'>公開\r\n";
        }
        if($is->images[0]->is_publish==0){
            print "			        <input type='radio' name='is_publish' value='0' checked>制限付き\r\n";
        }else{
            print "			        <input type='radio' name='is_publish' value='0'>制限付き\r\n";
        }

        print "			        <input type='button' id='changePhotoPublish' value='更新' onclick='funcChangePhotoPublish()'>\r\n";
        print "			    </span>\r\n";

		if(!$is->images[0]->is_publish){
			print "			    <span class='not_publish'>制限付き</span>\r\n";
		}
		print "			</p>\r\n";
	}

	$show_image_size_x = 800;
	$show_image_size_y = 600;
	/*
	if($is->images[0]->image_size_x < 800 && $is->images[0]->image_size_y < 600){
		$show_image_size_x = $is->images[0]->image_size_x;
	...
	if(!empty($is->images[0]->update_user_name)){
        print "				<dl>\r\n";
        print "					<dt>更新ユーザー</dt>\r\n";
        print "					<dd>".dp($is->images[0]->update_user_name)."</dd>\r\n";
        print "				</dl>\r\n";
        print "				<dl>\r\n";
        print "					<dt>更新日付</dt>\r\n";
        print "					<dd>".dp($is->images[0]->update_date)."</dd>\r\n";
        print "				</dl>\r\n";
    }
