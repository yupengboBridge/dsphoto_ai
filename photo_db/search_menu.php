...
//	} else if (obj.selectedIndex == 5) {
	...
	// BUD_PHOTO番号
		...
		//document.getElementById("search_bt").style.left = "530px";
		document.getElementById("search_bt").style.left = "525px";
	} else if(obj.selectedIndex == 15){
        document.getElementById("lbl_search_where").style.display="inline";
    } else {
		document.getElementById("lbl_search_where").style.display="inline";
    }
    if(obj.selectedIndex == 15){
        document.getElementById("search_where").setAttribute('disabled','true');
    }else{
        document.getElementById("search_where").removeAttribute('disabled');
    }
}
...
function go_search()
{
	...
	}else if(parseInt(selobj.selectedIndex) ==15){
        url = url + "?selIndex=15";
    }
			...
			<ul id="form_search">
				...
				<li>
					...
					<select id="p_hatsu" name="p_hatsu" style="width:200px; font-size:12px;" onchange="change_p_hatsu(this);">
                        ...
                        <?php if($s_group_id === 'BUD') {?>
                        <option value=''>制限付き</option>
                        <?php } ?>
					</select>
