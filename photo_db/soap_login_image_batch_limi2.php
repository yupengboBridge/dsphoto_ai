...
// WSDLのURL
//$wsdl = "https://".$_SERVER["SERVER_NAME"].dirname($_SERVER["PHP_SELF"])."/"."soap_login_image_batch_limi.wsdl";
$wsdl = "soap_login_image_batch_limi.wsdl";
...
function pc_sp($width,$height,$path){
	file_put_contents('/usr/local/apache/htdocs/photo_db/sp.txt', 
			'pc_sp start'.PHP_EOL.
			'pc_sp width==='.$width.PHP_EOL.
			'pc_sp height==='.$height.PHP_EOL.
			'pc_sp path==='.$path[0].PHP_EOL
			, FILE_APPEND);
	$db_link = db_connect();
	#长宽判断条件，生成SP
		if(1==1){
			...
			"UPDATE 
				photoimg 
			set 
				photo_filename_th11 = '{$set_pd_url}',
				is_pc = '1' 
			where
				photo_filename = '{$path[0]}'";
			$stmt = $db_link->prepare($sql);
			$result = $stmt->execute();
			#判断最后更新日期
			#存入数据库 is_sp
			// echo '原图片='.$original_photo.'</br>';
			// echo '原url1='.$url.'</br>';
			// echo '新路径+图片名称='.$new_path_name.'</br>';
			// echo '图片名称='.$file_name.'</br>';
			// echo '新路径='.$new_path.'</br>';
			// echo '原url='.$path[0].'</br>';
			// echo '写入数据库的='.$set_pd_url.'</br>';
			file_put_contents('/usr/local/apache/htdocs/photo_db/sp.txt', 
			'原图片='.$original_photo.PHP_EOL.
			'原url1='.$url.PHP_EOL.
			'新路径+图片名称='.$new_path_name.PHP_EOL.
			'图片名称='.$file_name.PHP_EOL.
			'新路径='.$new_path.PHP_EOL.
			'原url='.$path[0].PHP_EOL.
			'写入数据库的='.$set_pd_url.PHP_EOL.
			'result-success=='.$result.PHP_EOL
			,FILE_APPEND);
		}
	#长宽判断条件，生成PC
		if(2==2){
			...
			"UPDATE 
				photoimg 
			set 
				photo_filename_th12 = '{$set_pd_url}',
				is_sp = '1' 
			where
				photo_filename = '{$path[0]}'";
			$stmt = $db_link->prepare($sql);
...
{
	file_put_contents('/home/xhankyu/public_html/photo_db/soap_login_image_batch_limi.txt', 'uploadfilesstart'.PHP_EOL,FILE_APPEND);
	    file_put_contents('/home/xhankyu/public_html/photo_db/20230301c.txt', $csvcontent."--------------".PHP_EOL,FILE_APPEND);
    file_put_contents('/home/xhankyu/public_html/photo_db/20230301l.txt', $s_logininfo."--------------".PHP_EOL,FILE_APPEND);
	...
	try
	{
		...
		// PhotoImageのインスタンスを生成します。
		$pi = new PhotoImageDB ();
		file_put_contents('/home/xhankyu/public_html/photo_db/soap_login_image_batch_limi.txt', 'db'.PHP_EOL,FILE_APPEND);
	}
	catch(Exception $ex)
	{
		return "ERR_DB";
	...
	$csv_content = explode("\t",$csvcontent);
	...
	{
		file_put_contents('/home/xhankyu/public_html/photo_db/soap_login_image_batch_limi.txt', 'start'.PHP_EOL,FILE_APPEND);
		// --------画像管理番号を作成する（開始）------------------------------
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[5]".$csv_content[5],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[4]".$csv_content[4],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[6]".$csv_content[6],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[11]".$csv_content[11],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[12]".$csv_content[12],FILE_APPEND);
	...
	// 掲載期間（To）
	$dto = $csv_content[14];
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[14]".$csv_content[14],FILE_APPEND);
	if (!empty($dto))
	{
		$data_ary['p_dto'] = $dto;
	} else {
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[15]".$csv_content[15],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[16]".$csv_content[16],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[18]".$csv_content[18],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[19]".$csv_content[19],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[20]".$csv_content[20],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[22]".$csv_content[22],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[24]".$csv_content[24],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[25]".$csv_content[25],FILE_APPEND);
	...
	$tmp_photo_mno1_1 = $csv_content[32];//申請者管理番号
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[32]".$csv_content[32],FILE_APPEND);
	if (!empty($tmp_photo_mno1_1))
	{
		$tmp_photo_mno1 = sprintf("%05d",$tmp_photo_mno1_1);
	} else {
...
//yupengbo add start 2011/06/08
	$s_login_ary = array();
	$s_login_ary = explode(";",$s_logininfo);
...
//yupengbo add end   2011/06/08
	...
	//備考
	$note = $csv_content[31];
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "$csv_content[31]".$csv_content[31],FILE_APPEND);
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[21]".$csv_content[21],FILE_APPEND);
...
//		}
	...
	//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[7]".$csv_content[7],FILE_APPEND);
	if($p_classification_name1 == "海外")
	{
		// 方面ID
		$p_direction_name1 = $csv_content[8];
		//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[8]".$csv_content[8],FILE_APPEND);
		// 国・都道府県
		$p_country_prefecture_name1 = trimspace($csv_content[9]);
		//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[9]".$csv_content[9],FILE_APPEND);
		$tmp1 = getCountryPrefectureName($p_country_prefecture_name1);
		$p_country_prefecture_name1 = $tmp1;
		// 地名ID
		$p_place_name1 = $csv_content[10];
		//file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', "csv_content[10]".$csv_content[10],FILE_APPEND);
	} else {
		// 方面ID
		$p_direction_name1 = $csv_content[8];
		// 国・都道府県
	...
	$tmp = $csv_content[18];
	file_put_contents('/home/xhankyu/public_html/photo_db/soap_login_image_batch_limi.txt', 'finish'.PHP_EOL,FILE_APPEND);
	if (!empty($filename) && !empty($tmp))
	{
		uploadfile($filename,$tmp,$data_ary);
		if ($okflg == false)
		{
			file_put_contents('/home/xhankyu/public_html/photo_db/soap_login_image_batch_limi.txt', 'tmp-!null-err'.PHP_EOL,FILE_APPEND);
			return "ERR_OKFLG1";
		} elseif ($okflg == true) {
			file_put_contents('/home/xhankyu/public_html/photo_db/soap_login_image_batch_limi.txt', 'tmp-!null-ok'.PHP_EOL,FILE_APPEND);
			return "OK";
		}
...
function uploadfile($filename,$additional_constraints1,$p_d_ary)
{
	...
	try
	{
		...
		if ($fl->result == false)
		{
			file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', 
			'upload fail'.PHP_EOL, FILE_APPEND);
			$okflg = false;
		}
		file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', 
			'upload success'.PHP_EOL, FILE_APPEND);
		// サムネイルを作成します。
		$fl->make_thumbfile();
		if ($fl->result == false)
		{
			file_put_contents('/usr/local/apache/htdocs/photo_db/aa.txt', 
			'result3==false'.PHP_EOL, FILE_APPEND);
			$okflg = false;
		}
		file_put_contents('/usr/local/apache/htdocs/photo_db/aa.txt', 
			'三个if后okflg=='.$okflg.PHP_EOL, FILE_APPEND);
		// DB保存用のデータを設定します。
		...
		file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', 
			'width'.$pi->image_size_x.PHP_EOL, FILE_APPEND);
		// 画像情報新規用のデータを設定します。
		set_insertdata($p_d_ary);
		file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', 
			'insert success'.PHP_EOL, FILE_APPEND);
		#add liucongxu 20211027
		file_put_contents('/usr/local/apache/htdocs/photo_db/awhu.txt', 
			$fl->up_url[0].PHP_EOL.
			$fl->img_width[0].PHP_EOL.
			$fl->img_height[0].PHP_EOL, FILE_APPEND);
		// DBにアップロードした画像情報を新規登録します。
		$pi->insert_data($db_link);
		#pc_sp($pi->img_width[0],$pi->img_height[0],$pi->up_url);
...
class FileUploadBatch
{
	...
	function upload($imagedir)
	{
		...
		file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', 
			'tmppath=='.$tmppath.PHP_EOL, FILE_APPEND);
		...
		file_put_contents('/usr/local/apache/htdocs/photo_db/write_test.txt', 
			'end=='.$imagedir.$this->filename.PHP_EOL, FILE_APPEND);
