...
class priceCheckCron{
	...
	function __construct($flg,$naigai) {
		...
		if($flg=='cron'){
			$this->setFlg ='cron';
			echo 'CRONstart：'.$time = date("Y/m/d H:i:s.")."\n";	
		}
		elseif($flg=='all'){
			$this->setFlg ='all';
		...
		$isZipEx = '/home/xhankyu/public_html/photo_db/webtool/PriceCheck/outdata_test/trabid_i.xlsx';
		$isExl = '/home/xhankyu/public_html/photo_db/webtool/PriceCheck/outdata_test/trabid_d.xlsx';
	...
	function makeOutData(){
		foreach($this->setDataAry as $n => &$ary){		
			...
			elseif($ary["naigai"]=='d'){
				foreach($this->getRqDataAry as $url=>$v){
					if($ary['URL(text)']==$url){
						if($this->setFlg=='all' || $this->setFlg='cron'){//国内cronは全件
							$ary['tour_price(text)']=$v['tour_price(text)'];
							$ary['thedate']=date("Y-m-d");
							break;
						}
