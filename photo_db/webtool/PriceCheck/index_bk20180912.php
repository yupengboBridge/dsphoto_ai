...
class priceCheck{
	...
	function __construct() {
		...
		$this->baseFileDir ='/home/xhankyu/public_html/photo_db/webtool/PriceCheck/data/';
	...
	function setData($obj){	
		...
		if($obj[2]['A']){
			if(strpos($obj[2]['A'],'海外') !== false){
				if(strpos($obj[2]['A'],'拠点なし_アジア/台湾') !== false ){
					$this->naigai = "taiwan";
				}
				else{
					$this->naigai = "i";
			...
			elseif(strpos($obj[2]['A'],'国内') !== false){
				$this->naigai = "d";
			}
		}
		...
		foreach($obj[1] as $i =>$key){
			...
			if(!empty($a)){
				...
				if($this->naigai =="taiwan" && $key=='元URL' && empty($Required)){
					$Required ='元URL';
				}
				elseif($this->naigai =="d" && $key=='専門店URL' && empty($Required)){
...
/*
			...
			if(is_array($dataAry) && count($dataAry) > 0){
				...
				foreach($dataAry as $k => $dataAryV){
					...
					if($this->naigai =="i" || $this->naigai =="taiwan"){	
