...
function js_alter_history($message){
...
class FileUpload
{
	...
	function upload()
	{
		...
		{
			$this->result = false;
			$this->message = "ファイルのアップロードに失敗しました。（move_upload_fileエラー)";
			throw new Exception($this->message);
		}
	}

	function onlyLoad($physicalPath,$image)
	{
		// チェックでエラーが発生している場合は、例外をスローします。
		if ($this->result == false)
		{
		...
		$type = exif_imagetype($physicalPath);

		if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG)
		{
			$this->svname = $only_file_name = pathinfo($image, PATHINFO_FILENAME);
			$this->dirno = rand(0, 9);											// ディレクトリ名（0-9）をランダムで決定します。
			$this->dirno .= "/";

			// ファイルタイプがGIF、JPEG、PNGだった場合はテンポラリー→アップロードディレクトリにファイルを移動します。
			$this->svfullpath = array();
			$this->svfullpath[] = substr($image,1);
	...
	function make_thumbfile()
	{
		...
		for ($i = 0 ; $i < $szmax ; $i++)
		{
			if((int)$i == 3 || (int)$i == 2 || (int)$i == 5)
			{
				...
				if($thumb_width == 0 || (int)$width < $thumb_width)
				{
					$thumb_width = $width;
				}
...
class FileUploadBatchMall
{
	...
	function __construct($fln,$cf, $thdir, $flw, $fname, $cre, $wcredit, $image_root_dir, $photo_db_root_dir)
	{
		...
		$this->upfile = $fln;
		// アップロードするファイル名から拡張子を抜出します。
		preg_match("/\.[^.]*$/i", $this->upfile, $ext_tmp);
		// 拡張子を小文字に変換します。
		$this->ext = strtolower($ext_tmp[0]);
		$this->image_root_dir = $image_root_dir;
		$this->photo_db_root_dir = $photo_db_root_dir;
	}
	...
	function write_credit($img, $cre_str, $fsize, $width_i, $height_i)
	{
		...
		if($str_len > 20)
		{
			for($i=2;$i>0;$i--)
			{
				...
				if($i==2) $tmp_telop_text = mb_substr($telop_text,0,20,"utf-8");
				if($i==1) $tmp_telop_text = mb_substr($telop_text,20,20,"utf-8");
	...
	function upload()
	{
		...
		if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG)
		{
			...
			$this->convertToWebp($fullPath, $webpPath, $ext);
			// ファイルをWebPに変換します。end
		}
		else
		{
...
class PhotoImageData
{
	...
	function set_data($imgdata)
	{
		...
		$this->mall_no = $imgdata['mall_no'];																		// MALL番号
...
class PhotoImageDB extends PhotoImageData
{
	...
	/*
	 * 関数名：write_imagetodb_for_mall
	 * 関数説明：画像をバイナリになって、DBに保存する
	 * パラメタ：db_link：ＤＢリンク
	 * 戻り値：無し
	 */
	/*function write_imagetodb_for_mall($db_link,$p_photo_id)
	{
		if (empty($p_photo_id) || $p_photo_id == "-1" || $p_photo_id <= 0) return;
		...
		if ($result == true)
		{
			// 画像をバイナリをなる
			$tmp = substr($this->photo_filename,strpos($this->photo_filename,"./"));
			$b_image1 = file_get_contents($this->photo_db_root_dir."/".$tmp);

			$tmp = substr($this->photo_filename_th1,strpos($this->photo_filename_th1,"./"));
			$b_image2 = file_get_contents($this->photo_db_root_dir."/".$tmp);

			$tmp = substr($this->photo_filename_th2,strpos($this->photo_filename_th2,"./"));
			$b_image3 = file_get_contents($this->photo_db_root_dir."/".$tmp);

			$tmp = substr($this->photo_filename_th4,strpos($this->photo_filename_th4,"./"));
			$b_image5 = file_get_contents($this->photo_db_root_dir."/".$tmp);
			...
			if ((int)$tmpcnt > 0)
			{
				...
				try
				{
					...
					if ($result == false)
					{
						// 例外をスローします。
						$err = $stmt->errorInfo();
						$errorString = implode("|", $err);
						$this->message = "画像データの更新に失敗しました。（条件設定エラー）write_imagetodb_for_mall:::update:::".$errorString;
						throw new Exception($err[2]);
					}
				} catch(Exception $e) {
					// 例外をスローします。
				...
				try
				{
					...
					if ($result == false)
					{
						...
						$err = $stmt->errorInfo();
						$errorString = implode("|", $err);
						$this->message = "画像データの更新に失敗しました。（条件設定エラー）write_imagetodb_for_mall:::insert:::".$errorString;
						throw new Exception($err[2]);
					}
				} catch(Exception $e) {
					// 例外をスローします。
			...
			$this->message = "画像データの更新に失敗しました。（条件設定エラー）write_imagetodb_for_mall:::select";
			throw new Exception($this->message);
		}
	}*/
	...
	function write_imagetodb_for_mall($db_link,$p_photo_id)
	{
		...
		if ($result == true)
		{
					...
					try
					{
						...
						if($result == true)
						{
							...
							if ($attempt >= $maxRetries) 
							{
								...
								$err = $stmt->errorInfo();
								$errorString = implode("|", $err);
								$this->message = "画像データの新規に失敗しました。（条件設定エラー）write_imagetodb_for_mall:::insert:::".$errorString;
							}
						}
					} catch(Exception $e) {
						// 例外をスローします。
			...
			$this->message = "画像データの検索に失敗しました。（条件設定エラー）write_imagetodb_for_mall:::select";
			throw new Exception($this->message);
		}
	}
...
//		$sql .= " WHERE photo_id=" . $this->photo_id;
		...
		{
			$stmt->bindParam(20,$this->nopermis);
		} else {
			$stmt->bindValue(20,null);
		}
		if(!empty($this->nopermis_date))
		{
			$stmt->bindParam(21,$this->nopermis_date);
		} else {
			$stmt->bindValue(21,null);
		}
		if(!empty($this->nopermis_personid))
		{
			$stmt->bindParam(22,$this->nopermis_personid);
		} else {
			$stmt->bindValue(22,null);
		}
		if(!empty($this->is_publish))
		{
			$stmt->bindParam(23,$this->is_publish);
		} else {
		...
		{
			$stmt->bindParam(24,$this->photo_id);
		} else {
	...
	/*
	 * 関数名：update_data_for_mall
	 * 関数説明：ＤＢの更新
	 * パラメタ：db_link：ＤＢリンク
	 * 戻り値：無し
	 */
	function update_data_for_mall($db_link)
	{
		// 写真データを更新します。
		$sql = "UPDATE photoimg SET is_publish=? WHERE photo_id=?";
		$stmt = $db_link->prepare($sql);
		$stmt->bindParam(1,$this->is_publish);
	...
	/*
	 * 関数名：update_data_for_kikan
	 * 関数説明：ＤＢの更新
	 * パラメタ：db_link：ＤＢリンク
	 * 戻り値：無し
	 */
	function update_data_for_kikan($db_link)
	{
		// 写真データを更新します。
		$sql = "UPDATE photoimg SET dto=DATE_ADD(dto, INTERVAL 3 YEAR) WHERE photo_id=?";
		$stmt = $db_link->prepare($sql);
		$stmt->bindParam(1,$this->photo_id);
		// トランザクションを開始します。（オートコミットがオフになります。）
		$db_link->beginTransaction();
		try
		{
	...
	function update_thumb_1_10($db_link){
		// 写真サムネイルデータを更新します。
		$sql = "UPDATE photoimg SET ";
		$sql .= "photo_filename=?,";
		...
		if(!empty($this->photo_filename_th1))
		{
			$stmt->bindParam(2,$this->photo_filename_th1);
		} else {
			$stmt->bindValue(2,null);
		}
		if(!empty($this->photo_filename_th2))
		{
			$stmt->bindParam(3,$this->photo_filename_th2);
		} else {
			$stmt->bindValue(3,null);
		}
		if(!empty($this->photo_filename_th3))
		{
			$stmt->bindParam(4,$this->photo_filename_th3);
		} else {
			$stmt->bindValue(4,null);
		}
		if(!empty($this->photo_filename_th4))
		{
			$stmt->bindParam(5,$this->photo_filename_th4);
		} else {
			$stmt->bindValue(5,null);
		}
		if(!empty($this->photo_filename_th5))
		{
			$stmt->bindParam(6,$this->photo_filename_th5);
		} else {
			$stmt->bindValue(6,null);
		}
		if(!empty($this->photo_filename_th6))
		{
			$stmt->bindParam(7,$this->photo_filename_th6);
		} else {
			$stmt->bindValue(7,null);
		}
		if(!empty($this->photo_filename_th7))
		{
			$stmt->bindParam(8,$this->photo_filename_th7);
		} else {
			$stmt->bindValue(8,null);
		}
		if(!empty($this->photo_filename_th8))
		{
			$stmt->bindParam(9,$this->photo_filename_th8);
		} else {
			$stmt->bindValue(9,null);
		}
		if(!empty($this->photo_filename_th9))
		{
			$stmt->bindParam(10,$this->photo_filename_th9);
		} else {
			$stmt->bindValue(10,null);
		}
		if(!empty($this->photo_filename_th10))
		{
			$stmt->bindParam(11,$this->photo_filename_th10);
		} else {
			$stmt->bindValue(11,null);
		}

		$stmt->bindParam(12,$this->photo_id);
	 ...
	 * @throws Exception
	 */
	function update_thumb_1_10_for_mall($db_link){
		// 写真サムネイルデータを更新します。
		$sql = "UPDATE photoimg SET ";
		$sql .= "photo_filename=?,";
		$sql .= "photo_filename_th1=?,";
		...
		try
		{
			...
			if ($result == false){
				$err = $stmt->errorInfo();
				$errorString = implode("|", $err);
				$this->message = "画像サムネイルの更新に失敗しました。（条件設定エラー）update_thumb_1_10_for_mall::".$errorString;
				throw new Exception($err[2]);
			}
		}
		catch(Exception $e)
	...
	/**
	 * 関数名：batch_update_data_for_mall
	 * 関数説明：一括更新
	 * パラメタ：db_link：ＤＢリンク
	 * 戻り値：無し
	 */
	function batch_update_data_for_mall($db_link,$is_update_thumb=false)
	{
		if(empty($this->take_picture_time_id)) $this->take_picture_time_id = 0;
		if(empty($this->take_picture_time2_id)) $this->take_picture_time2_id = 0;
		if(empty($this->borrowing_ahead_id)) $this->borrowing_ahead_id = 0;
		if(empty($this->range_of_use_id)) $this->range_of_use_id = 0;
		...
		if(!empty($this->source_image_no))
		{
			$stmt->bindParam(1,$this->source_image_no);
		} else {
			$stmt->bindValue(1,null);
		}
		if(!empty($this->photo_name))
		{
			$stmt->bindParam(2,$this->photo_name);
		} else {
			$stmt->bindValue(2,null);
		}
		if(!empty($this->photo_explanation))
		{
			$stmt->bindParam(3,$this->photo_explanation);
		} else {
			$stmt->bindValue(3,null);
		}
		if(!empty($this->content_borrowing_ahead))
		{
			$stmt->bindParam(4,$this->content_borrowing_ahead);
		} else {
			$stmt->bindValue(4,null);
		}
		if(!empty($this->use_condition))
		{
			$stmt->bindParam(5,$this->use_condition);
		} else {
			$stmt->bindValue(5,null);
		}
		if(!empty($this->additional_constraints1))
		{
			$stmt->bindParam(6,$this->additional_constraints1);
		} else {
			$stmt->bindValue(6,null);
		}
		if(!empty($this->customer_section))
		{
			$stmt->bindParam(7,$this->customer_section);
		} else {
			$stmt->bindValue(7,null);
		}
		if(!empty($this->customer_name))
		{
			$stmt->bindParam(8,$this->customer_name);
		} else {
			$stmt->bindValue(8,null);
		}
		if(!empty($this->note))
		{
			$stmt->bindParam(9,$this->note);
		} else {
			$stmt->bindValue(9,null);
		}
		if(!empty($this->dto))
		{
			$stmt->bindParam(10,$this->dto);
		} else {
			$stmt->bindValue(10,null);
		}
		if(!empty($this->kikan))
		{
			$stmt->bindParam(11,$this->kikan);
		} else {
			$stmt->bindValue(11,null);
		}
		if(!empty($this->is_extension))
		{
			$stmt->bindParam(12,$this->is_extension);
		} else {
			$stmt->bindValue(12,null);
		}
		if(!empty($this->bud_photo_no))
		{
			$stmt->bindParam(13,$this->bud_photo_no);
		} else {
			$stmt->bindValue(13,null);
		}
		...
		try
		{
			...
			if ($result == true)
			{
				...
				if($is_update_thumb){
					...
					$this->update_thumb_1_10_for_mall($db_link);
				}
			...
			{
				$this->message = "画像データの更新に失敗しました。(batch_update_data_for_mall_no_th)";
				$errorInfo = $stmt->errorInfo();
				if(isset($errorInfo[2])){
					$this->message .= ":::".$errorInfo[2];
					$this->message .= ":::".$sql;
					$this->message .= ":::".$this->bud_photo_no;
					$this->message .= ":::".$this->source_image_no;
					$this->message .= ":::".$this->photo_name;
					$this->message .= ":::".$this->photo_explanation;
					$this->message .= ":::".$this->content_borrowing_ahead;
					$this->message .= ":::".$this->use_condition;
					$this->message .= ":::".$this->additional_constraints1;
					$this->message .= ":::".$this->customer_section;
					$this->message .= ":::".$this->customer_name;
					$this->message .= ":::".$this->note;
					$this->message .= ":::".$this->dto;
					$this->message .= ":::".$this->kikan;
					$this->message .= ":::".$this->is_extension;
					$this->message .= ":::".$this->photo_id;
				}
				return false;
			}
		}
	 ...
	 * 戻り値：無し
	 */
	function batch_update_data_for_mall_2($db_link)
	{
		try
		{
			if ((int)$this->publishing_situation_id == 2)
	...
	/**
	 * 関数名：batch_insert_data_for_mall
	 * 関数説明：一括更新
	 * パラメタ：db_link：ＤＢリンク
	 * 戻り値：無し
	 */
	function batch_insert_data_for_mall($db_link)
	{
		...
		$sql .= "`dto`,";
		$sql .= "`kikan`,";
		$sql .= "`take_picture_time_id`,";
		$sql .= "`take_picture_time2_id`,";
		$sql .= "`borrowing_ahead_id`,";
		$sql .= "`range_of_use_id`,";
		$sql .= "`registration_account`,";
		$sql .= "`registration_person`,";
		$sql .= "`register_date`,";
		$sql .= "`permission_account`,";
		$sql .= "`permission_person`,";
		$sql .= "`permission_date`,";
		...
		$stmt->bindParam(1,$this->photo_mno);
		$stmt->bindParam(2,$this->bud_photo_no);
		$stmt->bindParam(3,$this->photo_name);
		if(!empty($this->source_image_no))
		{
			$stmt->bindParam(4,$this->source_image_no);
		} else {
			$stmt->bindValue(4,null);
		}
		if(!empty($this->photo_explanation))
		...
		if(!empty($this->content_borrowing_ahead))
		{
			$stmt->bindParam(6,$this->content_borrowing_ahead);
		} else {
			$stmt->bindValue(6,null);
		}
		if(!empty($this->use_condition))
		{
			$stmt->bindParam(7,$this->use_condition);
		} else {
			$stmt->bindValue(7,null);
		}
		if(!empty($this->additional_constraints1))
		{
			$stmt->bindParam(8,$this->additional_constraints1);
		} else {
			$stmt->bindValue(8,null);
		}
		if(!empty($this->customer_section))
		{
			$stmt->bindParam(9,$this->customer_section);
		} else {
			$stmt->bindValue(9,null);
		}
		if(!empty($this->customer_name))
		{
			$stmt->bindParam(10,$this->customer_name);
		} else {
			$stmt->bindValue(10,null);
		}
		if(!empty($this->note))
		{
			$stmt->bindParam(11,$this->note);
		} else {
			$stmt->bindValue(11,null);
		}
		$stmt->bindParam(12,$this->dfrom);
		$stmt->bindParam(13,$this->dto);
		$stmt->bindParam(14,$this->kikan);
		if(!empty($this->take_picture_time_id))
		{
			$stmt->bindParam(15,$this->take_picture_time_id);
		} else {
			$stmt->bindValue(15,-1);
		}
		if(!empty($this->take_picture_time2_id))
		{
			$stmt->bindParam(16,$this->take_picture_time2_id);
		} else {
			$stmt->bindValue(16,-1);
		}
		$stmt->bindParam(17,$this->borrowing_ahead_id);
		$stmt->bindParam(18,$this->range_of_use_id);
		$stmt->bindParam(19,$this->registration_account);
		$stmt->bindParam(20,$this->registration_person);
		$stmt->bindParam(21,$this->register_date);
		$stmt->bindParam(22,$this->permission_account);
		$stmt->bindParam(23,$this->permission_person);
		$stmt->bindParam(24,$this->permission_date);
		$stmt->bindParam(25,$this->is_mall);
		$stmt->bindParam(26,$this->is_extension);
		$stmt->bindParam(27,$this->publishing_situation_id);
		if(!empty($this->image_size_x))
		{
			$stmt->bindParam(28,$this->image_size_x);
		} else {
			$stmt->bindValue(28,null);
		}
		if(!empty($this->image_size_y))
		{
			$stmt->bindParam(29,$this->image_size_y);
		} else {
			$stmt->bindValue(29,null);
		}
		if(!empty($this->ext))
		{
			$stmt->bindParam(30,$this->ext);
		} else {
			$stmt->bindValue(30,null);
		}
		...
		try
		{
			...
			else
			{
				...
				$this->message = "画像データの新規に失敗しました。(batch_insert_data_for_mall)";
				return false;
			}
		}
		catch(Exception $e)
	...
	function update_data_batch($db_link)
	{
		...
		$stmt->bindParam(3, $this->permission_date);
		$stmt->bindParam(4, $this->photo_id);
		// トランザクションを開始します。（オートコミットがオフになります。）
		$db_link->beginTransaction();
...
//		$sql .= " WHERE photo_id=" . $this->photo_id;
	...
	{
		$sql = "SELECT photo_filename,photo_filename_th1,photo_filename_th2,photo_filename_th3,photo_filename_th4 FROM photoimg WHERE photo_id = ?";
		$stmt = $db_link->prepare($sql);
		$stmt->bindParam(1, $pid);
	...
	function insert_data($db_link)
	{
		...
		try
		{
			...
			if($this->is_extension=="0" || $this->is_extension=="1"){
				$stmt->bindParam(33, $this->is_extension);
			}else{
				$stmt->bindParam(33, 0);
			}

			if($this->is_publish=="0" || $this->is_publish=="1"){
				$stmt->bindParam(34, $this->is_publish);
			}else{
				$stmt->bindParam(34, 0);
			}

			if($this->is_mall=="0" || $this->is_mall=="1"){
				$stmt->bindParam(35, $this->is_mall);
			}else{
				$stmt->bindParam(35, 0);
			}
	...
	// 画像IDよりデータを削除する
	function delete_data($db_link, $p_photo_id, $flg="0")
	{
		$retFlg = false;
		if (!empty($p_photo_id))
		{
...
class PhotoImageDataAll extends PhotoImageData
{
	...
	function set_data($imgdata)
	{
		...
		$this->update_user_name  = $imgdata['update_user_name'];
...
class ImageSearch
{
	...
	function init_condition()
	{
		...
		$this->sp_nopermission_reason = "";				//add by jinxin 2012/02/10
...
//added by wangtongchao 2011-12-13 end
	...
	private function build_select_image_where($param_opts, $where){
		$sql_where = " AND ";
		if ($param_opts == false)
		{
			$sql_where = " WHERE ";
	...
	function select_image($db_link, $p_tmp_kikan)
	{
		...
		{
			$sql .= $this->build_select_image_where($opts, "photo_mno COLLATE utf8_bin LIKE ".$this->sp_photo_mno);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "photo_name COLLATE utf8_bin LIKE ".$this->sp_photo_name);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "photo_explanation COLLATE utf8_bin LIKE ".$this->sp_photo_explanation);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "photoimg.take_picture_time_id=" . $this->sp_take_picture_time_id);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "photoimg.take_picture_time2_id=".$this->sp_take_picture_time2_id);
			$opts = true;
		}
		...
		if (!empty($this->sp_content_borrowing_ahead))
		{
			...
			// 写真入手元ID
			$tmp_sql = "photoimg.borrowing_ahead_id=" . $tmp[0];
			if (count($tmp) > 1)
			{
				$tmp_sql .= " AND photoimg.content_borrowing_ahead COLLATE utf8_bin LIKE '%".$tmp[1]."%'";
			}
		...
		if (!empty($this->sp_range_of_use_id))
		{
			...
			$tmp_sql = "photoimg.range_of_use_id=".$tmp[0];
			if (count($tmp) > 1)
			{
				if (!empty($tmp[1]))
				{
					$tmp_sql .= " AND use_condition COLLATE utf8_bin LIKE '%".$tmp[1]."%'";
				}
			}
			$sql .= $this->build_select_image_where($opts, $tmp_sql);
			$opts = true;
		...
		{
			$sql .= $this->build_select_image_where($opts, "additional_constraints1 COLLATE utf8_bin LIKE ".$this->sp_additional_constraints1);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "additional_constraints2 COLLATE utf8_bin LIKE ".$this->sp_additional_constraints2);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "note COLLATE utf8_bin LIKE ".$this->sp_note);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "copyright_owner COLLATE utf8_bin LIKE ".$this->sp_copyright_owner);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "kikan ='".$this->sp_kikan."'");
			$opts = true;
		}
		...
		if (!empty($this->sp_dto) && $this->sp_dto != "0000-00-00 00:00:00")
		{
			$dt = $this->sp_dto;
			$dtto = $dt . " 23:59:59";
			$sql .= $this->build_select_image_where($opts, "dto <= '" .$dtto."'");
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "source_image_no COLLATE utf8_bin LIKE ".$this->sp_source_image_no);
			$opts = true;
		}
		...
		{
			$tmp_sql = "bud_photo_no COLLATE utf8_bin LIKE '%".$this->sp_bud_photo_no."%'";
			if ($this->sp_bud_photo_no == "_")
			{
				$tmp_sql = "bud_photo_no = NULL OR bud_photo_no = '' OR bud_photo_no = null OR bud_photo_no = \"\"";
			}
			$sql .= $this->build_select_image_where($opts, $tmp_sql);
			$opts = true;
		}
		...
		{
			$sql .= $this->build_select_image_where($opts, "registration_person COLLATE utf8_bin LIKE ".$this->sp_registration_person);
			$opts = true;
		}
		...
		if (!empty($this->sp_classification_id))
		{
			...
			$sql3 = $sql1.$sql2.")";
			// --------WHERE SQL文を構築する（終了）--------------------------------------
			$sql .= $this->build_select_image_where($opts, "photo_id IN (".$sql3.") ");
			$opts = true;
		}
		...
		if (!empty($this->sp_keyword_str))
		{
			...
			// --------WHERE SQL文を構築する（終了）--------------------------------------
			$sql .= $this->build_select_image_where($opts, "photo_id IN (".$sql1.") ");
			$opts = true;
		}
		...
		// 制限付き
		if (isset($this->is_publish))
		{
			if($s_group_id == 'BUD'){
				if($this->is_publish == 0){
					$sql .= $this->build_select_image_where($opts, " is_publish = 0 ");
					$opts = true;
				}
			}else{
				$sql .= $this->build_select_image_where($opts, " is_publish = 1 ");
				$opts = true;
			}
		}else{
			if($s_group_id !== 'BUD'){
		...
		if (!empty($this->sp_monopoly_use))
		{
			...
			$sql .= $this->build_select_image_where($opts, "monopoly_use = ".$only_use);
		    ...
		    if($this->sp_login_id != "admin") {
		        $tmp_sql = " (monopoly_use = 1 AND registration_account = '".$this->sp_login_id."'" ." OR monopoly_use = 0) ";
				$sql .= $this->build_select_image_where($opts, $tmp_sql);
		        $opts = true;
		    }
		}
		...
		// 条件を設定します。
		$sql .= $this->build_select_image_where($opts, " photoimg.publishing_situation_id = 2");
		...
		$sql .= " AND photo_server_flg = 0 ORDER BY photo_id DESC";
		...
		for ($i = 0 ; $i < $ed ; $i++)
		{
			if (!empty($sp_photo_id_a))
			{
				...
				if ($result == true)
				{
					...
					{
						$image_data = $stmt->fetch(PDO::FETCH_ASSOC);
						if(!empty($image_data['update_user'])){
							$user = new UserManger();
							$user->select_data($db_link,$image_data['update_user']);
							$image_data["update_user_name"] = $user->user_name;
						}
						// 画像データをセットします。
						$photo_idata = new PhotoImageDataAll();
						$photo_idata->set_data($image_data);
	...
	function select_image_registed_forDelete($db_link,$where="",$orderby="")
	{
	    // SQLをセットします。
	    $sql = $this->set_sql();
	    $sql .= " WHERE photoimg.publishing_situation_id = 2 AND photo_server_flg=0";
	...
	function select_image_nopermit($db_link,$where="",$orderby="permit_date")
	{
		global $table_log_name;
		// SQLをセットします。
		$sql = "SELECT * FROM photoimg WHERE photoimg.publishing_situation_id = 3 AND photo_server_flg = 0";
		if(!empty($where)) $sql .= $where;
		$sql .= " ORDER BY ".$orderby;
		//print $sql;exit;
		$tmpistart = (int)$this->istart;
...
//					print_r($image_data);			//add by jinxin 2012/02/07
	...
	{
		//added by wangtognchao 2012-02-14 begin
		...
		$sql .= " WHERE photoimg.publishing_situation_id = 1 AND photo_server_flg = 0";
