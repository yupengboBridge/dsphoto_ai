...
if ($result == true)
{
	...
	while($image_data = $stmt->fetch(PDO::FETCH_ASSOC))
	{
		...
		if ($result1 == true)
		{
			...
			if ((int)$tmpcnt > 0)
			{
				$update_sql = "UPDATE photo_imgdata SET image4 = ? WHERE photo_id = ".$p_photo_id;

				$photo_filename_th3 = $image_data['photo_filename_th3'];
				$tmp = substr($photo_filename_th3,strpos($photo_filename_th3,"./"));
				$b_image4 = file_get_contents($tmp);

				try
				{
					$stmt2 = $db_link->prepare($update_sql);
					$stmt2->bindValue(1, $b_image4, PDO::PARAM_LOB);
