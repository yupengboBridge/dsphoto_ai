...
$s_is_credit = array_get_value($_SESSION,'is_credit' ,0);
...
//		}
        ...
        $old_img = get_photo($db_link,$p_photo_id);
        if($old_img['is_mall']=="1"){
            $pi->photo_id = $p_photo_id;											// 画像ID
            $pi->is_publish = array_get_value($_POST,"is_publish",1);				// 公開状況
            $pi->update_data_for_mall($db_link);
            ...
            $update_flag = $pi->update_data($db_link);
            if($update_flag){
...
function get_photo($db_link,$par_p_mno){
    $sql = "select *  from photoimg where photo_id = '".$par_p_mno."'";
    $stmt = $db_link->prepare($sql);
    $result = $stmt->execute();
    while($image_data = $stmt->fetch(PDO::FETCH_ASSOC))
    {
...
//	$pi->dfrom  = date("Y-m-d");														// 掲載期間（From）
	$pi->dfrom  = array_get_value($_POST,"hidden_dfrom","");								// 掲載期間（From） modify by jinxin 2012/02/10
	...
	if(empty($p_photo_filename)){
        ...
        $old_img = get_photo($db_link,$p_photo_id);
        if($old_img['additional_constraints1'] != $pi->additional_constraints1){
            $parsedUrl = parse_url($old_img['photo_filename']);
            $rootPath = $_SERVER['DOCUMENT_ROOT'];
            $physicalPath = $rootPath . $parsedUrl['path'];
            if(is_file($physicalPath)){
                $uploadedFile = array(
                    'name' => basename($physicalPath),
                    'type' => mime_content_type($physicalPath),
                    'tmp_name' => $physicalPath,
                    'error' => 0,
                    'size' => filesize($physicalPath)
                );
                $fl = new FileUpload($uploadedFile, "", "", "", "", $pi->additional_constraints1, "");
                $fl->onlyLoad($physicalPath,$parsedUrl['path']);
                $thumb_imgs = $fl->make_thumbfile();
                $pi->photo_filename = $thumb_imgs[0];							// アップロードURL
                $pi->photo_filename_th1 = $thumb_imgs[1];						// サムネイル1
                $pi->photo_filename_th2 = $thumb_imgs[2];						// サムネイル2
                $pi->photo_filename_th3 = $thumb_imgs[3];						// サムネイル3
                $pi->photo_filename_th4 = $thumb_imgs[4];						// サムネイル4
                $pi->photo_filename_th5 = $thumb_imgs[5];						// サムネイル5
                $pi->photo_filename_th6 = $thumb_imgs[6];						// サムネイル6
                $pi->photo_filename_th7 = $thumb_imgs[7];						// サムネイル7
                $pi->photo_filename_th8 = $thumb_imgs[8];						// サムネイル8
                $pi->photo_filename_th9 = $thumb_imgs[9];						// サムネイル9
                $pi->photo_filename_th10 = $thumb_imgs[10];						// サムネイル10
            }
        }else {
        ...
        try
        {
            ...
            $fl = new FileUpload($_FILES['p_photo_filename'], "", "", "", "", $pi->additional_constraints1, "");
            // ファイルをアップロードします。
            $fl->upload();
            // サムネイルを作成します。
            $fl->make_thumbfile();
            ...
            $pi->photo_filename_th5 = $fl->up_url[5];						// サムネイル5
            $pi->photo_filename_th6 = $fl->up_url[6];						// サムネイル6
            $pi->photo_filename_th7 = $fl->up_url[7];						// サムネイル7
            $pi->photo_filename_th8 = $fl->up_url[8];						// サムネイル8
            $pi->photo_filename_th9 = $fl->up_url[9];						// サムネイル9
            $pi->photo_filename_th10 = $fl->up_url[10];						// サムネイル10
        }
        catch(Exception $e)
        {
            // アップロードしたファイルを削除します。
    ...
    $pi->is_publish = array_get_value($_POST,"is_publish",1);
}
...
function disp_image()
{
	...
	if (empty($pi->image2))
	{
		...
		$pi->up_url[3]=str_replace("/photo_image/","/photo_db/",$pi->up_url[3]);
...
function disp_photo_mno()
{
	...
	print "	<dl class=\"reg_manage_number reg_clear\">\r\n";
	print "		<dt>写真管理番号</dt>\r\n";
	...
	if(empty($p_mno))
	{
		print "		<dd><input name=\"reg_photo_mno\" type=\"text\" id=\"reg_photo_mno\" value=\"\" MaxLength=\"5\" size=\"10\" style=\"ime-mode:disabled\" disabled=\"disabled\"/></dd>\r\n";
	} else {
		if (!empty($tmp1))
		{
			$tmp2 = (int)$tmp1;
			...
			if ($tmp2 == 1)
			{
				print "		<dd><input name=\"reg_photo_mno\" type=\"text\" id=\"reg_photo_mno\" value=\"".$p_mno."\" MaxLength=\"5\" size=\"10\" style=\"ime-mode:disabled\" disabled=\"disabled\"/></dd>\r\n";
			// 掲載状況（掲載許可）
			} elseif ($tmp2 == 2) {
...
function disp_photo_file_name_upload(){
    global $pi;

    if($pi->is_mall == "0"){
        print "<dl class=\"reg_filedata reg_clear reg_list_none_top required\">\r\n";
        print "    <dt>画像ファイル</dt>\r\n";
        print "    <dd class=\"bt_reg_reference\" style=\"width: auto;\">\r\n";
...
function disp_publishing_situation($ps_id,$ps_name)
{
	...
	$is_disabled = "";
	if($pi->is_mall == "1") $is_disabled = "disabled";

	$ed = count($ps_id);

	print"<select id='reg_situation' name='reg_situation' onchange='change_reg_situation(this);' ".$is_disabled.">\r\n";
	for ($i=0;$i < $ed;$i++)
	{
...
function disp_photo_name()
{
	...
	print "		<dt>被写体の名称</dt>\r\n";
	print "		<dd><input name=\"reg_subject\" type=\"text\" id=\"reg_subject\" size=\"65\" value='".$pi->photo_name."' ".$is_disabled."/></dd>\r\n";
	print "	</dl>\r\n";
}
...
function registration_division($c_id, $c_name)
{
	...
	for ($i = 0 ; $i<$ed ; $i++)
	{
		...
		{
			print "	<label><input name='reg_division' type='radio' value=\"".$c_id[$i]."\" checked=\"checked\" ".$is_disabled." />".dp($c_name[$i])."</label>";
		} else {
			print "	<label><input name='reg_division' type='radio' value=\"".$c_id[$i]."\" ".$is_disabled." />".dp($c_name[$i])."</label>";
		}
	}
}
...
function disp_div_classification($c_id, $c_name, $no)
{
    ...
    $is_hide = "";
    if($pi->is_mall == "1") $is_hide = "style='display:none;'";
	...
	if(strpos($_SERVER["HTTP_USER_AGENT"], "MSIE 6.0"))
	{
		...
		print "<ul class=\"bt_plus_minasu\" ".$is_hide.">\r\n";
...
function disp_classification($c_id, $c_name, $no)
{
	...
	print "	<select name='" . $selid . "' id='" . $selid . "' ".$is_disabled.">\r\n";
...
function disp_category($cg_id,$cg_name)
{
	...
	for ($i=0;$i < $dc;$i++)
	{
		...
		{
			print "<input id='".$id."' type='checkbox' category='0' value='".dp($cg_name[$i][0])."' onclick='change_category(\"$id\");' checked=\"checked\" ".$is_disabled."/>&nbsp;".dp($cg_name[$i][0])."</em>";
		} else {
			print "<input id='".$id."' type='checkbox' category='0' value='".dp($cg_name[$i][0])."' onclick='change_category(\"$id\");' ".$is_disabled."/>&nbsp;".dp($cg_name[$i][0])."</em>";
		}
		...
		for($j = 1;$j < $dc2; $j++)
		{
			...
			if ($s_len >= 6)
			{
				...
				{
					print "<li class='wide140'><input id='".$id."' type='checkbox' category='0' value='".dp($cg_name[$i][$j])."' onclick='change_category(\"$id\");' checked=\"checked\" ".$is_disabled."/>&nbsp;".dp($cg_name[$i][$j])."</li>";
				} else {
					print "<li class='wide140'><input id='".$id."' type='checkbox' category='0' value='".dp($cg_name[$i][$j])."' onclick='change_category(\"$id\");' ".$is_disabled."/>&nbsp;".dp($cg_name[$i][$j])."</li>";
				}
			}else
			{
				// 登録したキーワードはキーワードの配列に存在した場合
				if (check_array_index($kwd_a,dp($cg_name[$i][$j])) != -1)
				{
					print "<li><input id='".$id."' type='checkbox' category='0' value='".dp($cg_name[$i][$j])."' onclick='change_category(\"$id\");' checked=\"checked\" ".$is_disabled."/>&nbsp;".dp($cg_name[$i][$j])."</li>";
				} else {
					print "<li><input id='".$id."' type='checkbox' category='0' value='".dp($cg_name[$i][$j])."' onclick='change_category(\"$id\");' ".$is_disabled."/>&nbsp;".dp($cg_name[$i][$j])."</li>";
				}
			}
		}
		print "</ul>";
...
function take_picture_time2($c_id, $c_name)
{
	...
	for ($i = 0 ; $i<$ed ; $i++)
	{
		...
		{
			print "	<label><input name='rad_kisetu' type='radio' value=\"".$c_id[$i]."\" checked=\"checked\" ".$is_disabled."/>".dp($c_name[$i])."</label>";
		} else {
			print "	<label><input name='rad_kisetu' type='radio' value=\"".$c_id[$i]."\" ".$is_disabled."/>".dp($c_name[$i])."</label>";
		}
	}
}
...
function take_picture_time($c_id, $c_name, $flg)
{
	...
	if ($flg == 0)
	{
		print"	<select id='take_picture_time_id' name='take_picture_time_name' ".$is_disabled.">";
	}
	...
	if ($flg == 1)
	{
		...
		if ($pi->kikan != "shitei")
		{
			...
			print"	<select id='take_picture_time_id' name='take_picture_time_name' onChange='calendar();' ".$is_disabled.">";
		}
	}
...
function disp_photo_explanation()
{
	...
	print "		<dd>\r\n";
	print "			<textarea name=\"reg_material_txt\" id=\"reg_material\" cols=\"70\" rows=\"5\" ".$is_disabled.">".$pi->photo_explanation."</textarea>\r\n";
	print "		</dd>\r\n";
	print "	</dl>\r\n";
}
...
function disp_kikan()
{
	...
	switch ($pi->kikan)
	{
		...
		case 'mukigen':
			...
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sankagetu\" onclick='change_kikan(this);' ".$is_disabled."/>3ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"hantoshi\" onclick='change_kikan(this);' ".$is_disabled."/>6ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ichinen\" onclick='change_kikan(this);' ".$is_disabled."/>1年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 begin
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ninen\" onclick='change_kikan(this);' ".$is_disabled."/>2年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sannen\" onclick='change_kikan(this);' ".$is_disabled."/>3年間 </label>\r\n";
		...
		case 'sankagetu':
			...
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sankagetu\" checked=\"checked\" onclick='change_kikan(this);' ".$is_disabled."/>3ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"hantoshi\" onclick='change_kikan(this);' ".$is_disabled."/>6ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ichinen\" onclick='change_kikan(this);' ".$is_disabled."/>1年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 begin
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ninen\" onclick='change_kikan(this);' ".$is_disabled."/>2年間 </label>\r\n";
		...
		case 'hantoshi':
			//print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"mukigen\" onclick='change_kikan(this);'/>無期限</label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sankagetu\" onclick='change_kikan(this);' ".$is_disabled."/>3ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"hantoshi\" checked=\"checked\" onclick='change_kikan(this);' ".$is_disabled."/>6ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ichinen\" onclick='change_kikan(this);' ".$is_disabled."/>1年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 begin
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ninen\" onclick='change_kikan(this);' ".$is_disabled."/>2年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sannen\" onclick='change_kikan(this);' ".$is_disabled."/>3年間 </label>\r\n";
		...
		case 'ichinen':
			...
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sankagetu\" onclick='change_kikan(this);' ".$is_disabled."/>3ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"hantoshi\" onclick='change_kikan(this);' ".$is_disabled."/>6ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ichinen\" checked=\"checked\" onclick='change_kikan(this);' ".$is_disabled."/>1年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 begin
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ninen\" onclick='change_kikan(this);' ".$is_disabled."/>2年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sannen\" onclick='change_kikan(this);' ".$is_disabled."/>3年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 end
		...
		case 'ninen':
			...
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"hantoshi\" onclick='change_kikan(this);' ".$is_disabled."/>6ヵ月 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ichinen\" onclick='change_kikan(this);' ".$is_disabled."/>1年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ninen\" checked=\"checked\" onclick='change_kikan(this);' ".$is_disabled."/>2年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sannen\" onclick='change_kikan(this);' ".$is_disabled."/>3年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 end
			break;
			//added by wangtongchao 2011-12-02 begin
		...
		case 'sannen':
			...
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ichinen\" onclick='change_kikan(this);' ".$is_disabled."/>1年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"ninen\" onclick='change_kikan(this);' ".$is_disabled."/>2年間 </label>\r\n";
			print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"sannen\" checked=\"checked\" onclick='change_kikan(this);' ".$is_disabled."/>3年間 </label>\r\n";
			//added by wangtongchao 2011-12-02 end
			break;
		// 日付指定
		default:
...
function disp_kikan2()
{
	...
	if ($pi->kikan == "shitei")
	{
		print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"shitei\" checked=\"checked\" onclick='change_kikan(this);' ".$is_disabled."/>日付指定</label>\r\n";
	} else {
		print "		<label><input name=\"reg_pub_period\" type=\"radio\" value=\"shitei\" onclick='change_kikan(this);' ".$is_disabled."/>日付指定</label>\r\n";
	}
	print "			<label>\r\n";
	take_picture_year();
	print "			</label>\r\n";
...
function take_picture_year()
{
	...
	if ($pi->kikan != "shitei")
	{
		...
		print"<select name=\"select_year\" id=\"select_year\" ".$is_disabled.">\r\n";
	}
...
function take_pictrue_day()
{
	...
	if ($pi->kikan != "shitei")
	{
		...
		print"	<select name=\"select_day\" id=\"select_day\"".$is_disabled.">\r\n";
	}
...
function disp_range($r_id,$r_name)
{
	...
	for ($i=0;$i < $ed;$i++)
	{
		...
		if ((int)$r_id[$i] == 3)
		{
			...
			{
				print"		<label><input name=\"reg_pub_possible\" type=\"radio\" value=".$r_id[$i]." checked=\"checked\" onclick='change_range_radio(this);' ".$is_disabled."/>".dp($r_name[$i])."</label>\r\n";
				print"		<input name=\"reg_pub_possible_txt\" type=\"text\" id=\"reg_pub_possible_txt\" size=\"30\" onblur='return check_reg_pub_possible(this);' value='".$pi->use_condition."' ".$is_disabled."/>\r\n";
			} else {
				print"		<label><input name=\"reg_pub_possible\" type=\"radio\" value=".$r_id[$i]." onclick='change_range_radio(this);' ".$is_disabled."/>".dp($r_name[$i])."</label>\r\n";
...
{
	global $pi, $s_is_credit;

    $is_disabled = "";
    if($pi->is_mall == "1") $is_disabled = "disabled";
    if($s_is_credit == 0 || empty($s_is_credit))  $is_disabled = "disabled";
	...
	{
		print "		<dd>\r\n";
		print "			<label><input name=\"reg_addition\" id=\"reg_addition2\" type=\"radio\" checked=\"checked\" value=\"2\" onclick='change_reg_addition(this);' ".$is_disabled."/>なし </label>\r\n";
		print "		</dd>\r\n";
		//print "		<dd style=\"height:14px\"></dd>\r\n";
		print "		<dd class=\"outside\">\r\n";
		print "			<label><input name=\"reg_addition\" type=\"radio\" value=\"0\" onclick='change_reg_addition(this);' ".$is_disabled."/>要クレジット </label>\r\n";
		//yupengbo modify 20101208 start
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt0\" size=\"30\" ".$is_disabled."/>\r\n";
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt1\" size=\"30\" ".$is_disabled."/>\r\n";
		//yupengbo modify 20101208 end
		print "		</dd>\r\n";
		print "		<dd class=\"outside\">\r\n";
		print "			<label><input name=\"reg_addition\" type=\"radio\" value=\"1\" onclick='change_reg_addition(this);' ".$is_disabled."/>要使用許可 </label>\r\n";
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt2\" size=\"30\" ".$is_disabled."/>\r\n";
		print "		</dd>\r\n";
		print "	</dl>\r\n";
		return;
	}
	...
	print "		<dd>\r\n";
	print "			<label><input name=\"reg_addition\" id=\"reg_addition2\" type=\"radio\" value=\"2\"  onclick='change_reg_addition(this);' ".$is_disabled."/>なし </label>\r\n";
	print "		</dd>\r\n";
	//print "		<dd style=\"height:14px\"></dd>\r\n";
	...
	if (!empty($pi->additional_constraints1) && $pi->additional_constraints1 != "")
	{
		...
		print "			<label><input name=\"reg_addition\" type=\"radio\" value=\"0\" onclick='change_reg_addition(this);' checked=\"checked\" ".$is_disabled."/>要クレジット </label>\r\n";
		...
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt0\" size=\"30\" value='".$additional_constraints1[0]."' ".$is_disabled."/>\r\n";
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt1\" size=\"30\" value='".$additional_constraints1[1]."' ".$is_disabled."/>\r\n";
	...
	{
		print "			<label><input name=\"reg_addition\" type=\"radio\" value=\"1\" onclick='change_reg_addition(this);' checked=\"checked\" ".$is_disabled."/>要使用許可 </label>\r\n";
		//changed by wangtongchao 2011-12-16 begin
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt2\" size=\"30\" value='".$pi->additional_constraints2."' ".$is_disabled."/>\r\n";
		//changed by wangtongchao 2011-12-16 end
	} else {
		print "			<label><input name=\"reg_addition\" type=\"radio\" value=\"1\" onclick='change_reg_addition(this);' ".$is_disabled."/>要使用許可 </label>\r\n";
		print "			<input name=\"reg_addition_txt\" type=\"text\" id=\"reg_addition_txt2\" size=\"30\" ".$is_disabled."/>\r\n";
...
function disp_monopoly_use()
{
	...
	{
		print "			<label><input name=\"reg_account\" type=\"checkbox\" value=\"1\" checked=\"checked\" ".$is_disabled."/>この申請アカウント </label>\r\n";
	} else {
		print "			<label><input name=\"reg_account\" type=\"checkbox\" value=\"1\" ".$is_disabled."/>この申請アカウント </label>\r\n";
	}
	print "		</dd>\r\n";
	print "	</dl>\r\n";
}
	...
	for ($i=0;$i < $ed;$i++)
	{
		...
		if ((int)$b_head_id[$i] == 2)
		{
			...
			if ((int)$pi->borrowing_ahead_id == (int)$b_head_id[$i])
			{
				...
				print"		<input name=\"reg_p_obtaining_txt\" type=\"text\" id=\"reg_p_obtaining_txt\" size=\"30\" onblur='check_borrowing_ahead(this);' value='".$pi->content_borrowing_ahead."' ".$is_disabled."/>\r\n";
			} else {
				print"		<input name=\"reg_p_obtaining\" type=\"radio\" value=".$b_head_id[$i]." onclick='change_obtaining_radio(this);' checked=\"checked\" style=\"display:none\" />";
				print"		<input name=\"reg_p_obtaining_txt\" type=\"text\" id=\"reg_p_obtaining_txt\" size=\"30\" ".$is_disabled."/>\r\n";
			}
			print"	</dd>\r\n";
		} else {
			// 写真入手元は初期表示ではない場合
...
function disp_copyright_owner()
{
	...
	print "		<dt>版権所有者</dt>\r\n";
	print "		<dd><input name=\"reg_copyright\" type=\"text\" id=\"reg_copyright\" size=\"30\" value='".$pi->copyright_owner."' ".$is_disabled."/></dd>\r\n";
	print "	</dl>\r\n";
}
...
function disp_source_image_no()
{
	...
	print "		<dt>素材管理番号</dt>\r\n";
	print "		<dd><input name=\"reg_mate_mana\" type=\"text\" id=\"reg_mate_mana\" size=\"30\" value='".$pi->source_image_no."' ".$is_disabled."/></dd>\r\n";
	print "	</dl>\r\n";
}
...
function disp_bud_photo_no()
{
	...
	{
		print "			<label><input name=\"reg_bud_number\" type=\"radio\" value='1' checked=\"checked\" onclick=\"change_bud_number_radio(this);\" ".$is_disabled."/>ある </label>\r\n";
		//changed by wangtongchao 2011-12-16 begin
		print "			<input name=\"reg_bud_number_txt\" type=\"text\" id=\"reg_bud_number_txt\" size=\"30\" onblur='check_reg_bud_number(this);' value='".$pi->bud_photo_no."' style=\"ime-mode:disabled\" ".$is_disabled."/>\r\n";
		//changed by wangtongchao 2011-12-16 end
	} else {
		print "			<label><input name=\"reg_bud_number\" type=\"radio\" value='1' onclick=\"change_bud_number_radio(this);\" ".$is_disabled."/>ある </label>\r\n";
		print "			<input name=\"reg_bud_number_txt\" type=\"text\" id=\"reg_bud_number_txt\" size=\"30\" disabled=\"disabled\" style=\"ime-mode:disabled\" />\r\n";
	}
	//changed by wangtongchao 2011-12-16 end
	print "		</dd>\r\n";
	print "		<dd class=\"other\">\r\n";
	if (empty($pi->bud_photo_no))
	{
		print "			<label><input name=\"reg_bud_number\" type=\"radio\" value='0' checked=\"checked\" onclick=\"change_bud_number_radio(this);\" ".$is_disabled."/>なし </label>\r\n";
	} else {
		print "			<label><input name=\"reg_bud_number\" type=\"radio\" value='0' onclick=\"change_bud_number_radio(this);\" ".$is_disabled."/>なし </label>\r\n";
	}
	print "		</dd>\r\n";
	print "	</dl>\r\n";
}
...
function disp_customer()
{
	...
	print "			<dt>お客様情報</dt>\r\n";
	print "			<dd> 部署名\r\n";
	print "				<input name=\"post_name\" type=\"text\" id=\"post_name\" size=\"15\" value='".$pi->customer_section."' ".$is_disabled."/>\r\n";
	print "				名前\r\n";
	print "				<input name=\"first_name\" type=\"text\" id=\"first_name\" size=\"30\" value='".$pi->customer_name."' ".$is_disabled."/>\r\n";
	print "			</dd>\r\n";
	print "		</dl>\r\n";
}
...
function disp_note()
{
	...
	print "<p class=\"reg_remarks\"><textarea name=\"reg_remarks\" id=\"textarea2\" cols=\"70\" rows=\"5\" ".$is_disabled.">".dp($pi->note)."</textarea></p>\r\n";
}

function disp_is_publish()
{
    global $pi;
    //MALL画像のみ表示
    if($pi->is_mall == "1"){
        print "<div>\r\n";
        print " <h2>そのた設定</h2>\r\n";
        print "	<dl class=\"take_picture  reg_clear\">\r\n";
        print "		<dt style='height: auto'>公開状況</dt>\r\n";
        print "		<dd>\r\n";

        if ($pi->is_publish == 1)
        {
            print "			<label><input name=\"is_publish\" type=\"radio\" value='1' checked=\"checked\" />公開 </label>\r\n";
        } else {
            print "			<label><input name=\"is_publish\" type=\"radio\" value='1' />公開 </label>\r\n";
        }

        if ($pi->is_publish == 0)
        {
            print "			<label><input name=\"is_publish\" type=\"radio\" value='0' checked=\"checked\" />制限付き </label>\r\n";
        } else {
            print "			<label><input name=\"is_publish\" type=\"radio\" value='0' />制限付き </label>\r\n";
        }
        print "		</dd>\r\n";
        print "	</dl>\r\n";
        print "</div>\r\n";
...
function disp_nopermis($ps_id,$ps_name){
	...
	for($i=0;$i < $ed;$i++){
		if(($i+1)%6 == 0){
			print "<br /><label><input name='nopermisnote' type='radio' value=\"".$ps_name[$i]."\" onclick=\"change_noper_reason(this);\" ".$is_disabled."/>".$ps_name[$i]."</label>";
		}else{
			print"<label><input name='nopermisnote' type='radio' value=\"".$ps_name[$i]."\" onclick=\"change_noper_reason(this);\" ".$is_disabled."/>".$ps_name[$i]."</label>";
		}
	}
	print "<label><input name='nopermisnote' type='radio' value=\"7\" onclick=\"change_noper_reason(this);\" ".$is_disabled."/>その他 </label><input id=\"nopermis_rea_txt\" name=\"enterreason\" type=\"text\" size=\"40\" disabled=true/><input name=\"nopermis\" id=\"nopermis\" type=\"hidden\" value=\"\" ".$is_disabled."/>";
}

function disp_reg_submit_btn(){
    global $pi;

    $is_hide = "";
    if($pi->is_mall == "1") $is_hide = "style='display:none;'";

    print "<div class=\"reg_search_btn\">\r\n";
    print "    <ul>\r\n";
    print "        <li id=\"changedivli\" class=\"bt_reg_change\"><a href=\"#\" onclick=\"form_submit();return false;\" title=\"登録変更\">登録確認</a></li>\r\n";
    print "        <li class=\"bt_reg_delete\" ".$is_hide."><a href=\"#\" onclick=\"delete_record();\" title=\"削除\">削除</a></li>\r\n";
    print "        <li id=\"nopermisdivli\" class=\"bt_reg_nopermis\" ".$is_hide."><a href=\"#\" onclick=\"form_submit();return false;\" title=\"不許可\">不許可</a></li>\r\n";
    print "    </ul>\r\n";
    print "    <p>※続けて画像を登録申請する場合は、前の入力内容に上書き、もしくはクリアして入力してください</p>\r\n";
    print "</div>\r\n";
}
...
function clearInputFile() {
    var fileInput = document.getElementById('p_photo_filename');
    fileInput.value = '';
}
...
function init()
{
	...
	if(obj_frame) obj_frame.style.height = 0;
	var obj_frame = top.document.getElementById('iframe_bottom');
	if(obj_frame) obj_frame.style.height = 2700;
...
<body>
<form enctype="multipart/form-data" name="register_image_edit" action="" method="post">
<div id="zentai">
<div id="contents">
	<div id="registration">
		<div>
			...
			<div class="reg_file_subject">
                <?php  disp_photo_file_name_upload(); ?>
				<?php  disp_photo_mno(); ?>
		...
		<div>
			...
			<dl class="reg_pub_period reg_clear reg_list_none_top">
				...
				<?php  disp_kikan(); ?>
				<?php  disp_kikan2(); ?>
				<?php echo '<input type="hidden" id="hidden_dfrom" name="hidden_dfrom" value="'.$pi->get_dfrom_date_forhidden($db_link,$p_photo_id).'" />';?>
			</dl>
