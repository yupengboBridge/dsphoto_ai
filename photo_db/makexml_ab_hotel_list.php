...
$xmldir_hotel = "hotel/";
$xmldir_old_hotel =  dirname(__FILE__) . '/hotelxml';
...
function get_hatsu_ab_hotel_list()
{
	...
	if(!is_file($xmldir."i_p_hatsu_url_v3.xml")) return;

	$xml = simplexml_load_file($xmldir."i_p_hatsu_url_v3.xml");
...
//		$hotel_name = $csvarr[11];
		...
		$hotel_name = htmlspecialchars($hotel_name, ENT_QUOTES, 'UTF-8',false);
		...
		if(!empty($hotel_code))
		{
			...
			{
				if(is_array($hotel_arr[$country_code])){
					foreach($hotel_arr[$country_code] as $hotelKey => $hotelVal){
						if($hatsu_code.$hotel_code == $hotelVal)
						{
	...
	foreach($xml_arr as $country => $h_codeAry){
		if(is_array($h_codeAry)){
			foreach($h_codeAry as $h_code => $noAry){
				if(is_array($noAry)){
					foreach($noAry as $no => $srr){
					
					}
				}
				$xml_arr_new[$country][$h_code] = array_unique($noAry);
			}
		}
	}
ksort($xml_arr_new);
	...
	foreach($xml_arr_new as $key=>$value)
	{
		$filename = $key.".xml";
		$xml = "";
