...
$max_size_d_p_hatsu_url = 12;

//$flag = MakeXml_d_p_hatsu_url_v2("d_p_hatsu_url_v2.csv");
//if($flag)
//{
//	print "ＸＭＬを出力しました！\r\n";
//}
...
$pull_flg						= 11;
/*
 * 関数名：MakeXml_d_p_hatsu_url_v2
 * 関数説明：ｘｍｌを出力する（出発地URL カレンダー国内用　 検索国内共有）
 * パラメタ：filename　CSVファイル名
 * 戻り値：無し　
 */
function MakeXml_d_p_hatsu_url_v2($filename)
{
	...
	LoadJyogaiCSV_d_p_hatsu_url_v2($filename01);
	...
	while(! feof($file))
	{
		...
		if($row != 1 && $cnt > 0)
		{
			...
			{
				$exists_flg = CheckExists_d_p_hatsu_url_v2($csvarr);
				if($exists_flg)
				{
					continue;
				}
...
/*
 * 関数名：CheckExists_d_p_hatsu_url_v2
 * 関数説明：除外リストにあるかどうかチェックする
 * パラメタ：src_line　読み込みのCSVデータ
 * 戻り値：true/false　
 */
function CheckExists_d_p_hatsu_url_v2($src_line)
{
...
/*
 * 関数名：LoadJyogaiCSV_d_p_hatsu_url_v2
 * 関数説明：除外CSVを読み込む
 * パラメタ：filename　CSVファイル名
 * 戻り値：無し　
 */
function LoadJyogaiCSV_d_p_hatsu_url_v2($filename)
{
