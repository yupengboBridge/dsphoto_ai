...
$content=file_get_contents('http://xzdb.hstech-china.com/photo_db/./thumb12/6/202110291159381096.jpg');
$content=file_get_contents('http://xzdb.hstech-china.com/cms_photo_image/thumb1/6/20211015154310723th1.webp');
// echo '<img src="http://xzdb.hstech-china.com/photo_db/./thumb12/6/202110291159381096.jpg">';
// echo 'http://x.hankyu-travel.com/photo_db/image_search_kikan5.php?p_photo_mno=00054-ALLUP-152172.jpg';
echo $content;
	...
	if ($result == true)
	{
		...
		if ($icount > 0)
		{
			...
			if ($now >= $img['dfrom'] && $now <= $img['dto'])
			{
				...
				$p_id = $img['photo_id'];
				$sql1 = "select * from photo_imgdata where photo_id=?";

				$stmt1 = $db_link->prepare($sql1);
				$stmt1->bindParam(1, $p_id);
				// SQLを実行します。
				...
				if ($icount1 > 0)
				{
					$img1 = $stmt1->fetch(PDO::FETCH_ASSOC);
					$tmp1 = $img1['image3'];
//liukeyu add strat 20110728
					...
					//yupengbo modify end 20120203
						...
						// $content=file_get_contents('http://xzdb.hstech-china.com/cms_photo_image/./thumb2/2/20211013163512318th2.webp');
						// echo $img['photo_filename_th2'];
						$l_ext = array_pop(explode('.',$img['photo_filename_th2']));
						if ($l_ext == 'jpg'){
							$l_webp_path = str_ireplace('.jpg', '.webp', $img['photo_filename_th2']);
							// echo $l_webp_path;
							// echo $_SERVER['SERVER_NAME'];
							$l_jpg_file_path = '../'.explode($_SERVER['SERVER_NAME'], $img['photo_filename_th2'])[1];
...
function getNewImageName($fileName,$width,$height)
{
	...
	//$type = "gif";
	$ary_name = explode("-",basename($fileName,".".$type));//split
	$tmp_name = "";
