...
$csv_file_name = "cms_photo_data_all_list.csv";
...
function export_csv()
{
    ...
    if($db_link) {
        $sql="select * from photoimg where publishing_situation_id = 2 and photo_server_flg = 1";
...
function sendMail($isSucess){
    ...
    $mailaddresses = array("nakao@bridge.vc","jiangtao@bridge.vc","tanaka@bridge.vc ", "yaolili@bridge.vc");
    //$mailaddresses = array("wangjingdong@bridge.vc", "jiangtao@bridge.vc", "yaolili@bridge.vc");


    $headers['Date'] = date("r");

    $subject =      "cms_photo_data_all_list作成結果";
    $subject_b = mb_convert_encoding($subject,"JIS","UTF-8");
    $subject_f = "=?ISO-2022-JP?B?" . base64_encode($subject_b) . "?=";
    $body = "";
    if($isSucess == "OK"){
        $body = <<<EOS
cms_photo_data_all_listの作成に成功しました。
EOS;
    }else {
        $body = <<<EOS
cms_photo_data_all_listの作成に失敗しました。
EOS;
    }
