 ...
 * @throws Exception
 */
function update_photo_filename_th($path, $percent){
...
function resize_image($filename,$percent,$new_path)
{
	...
	switch ($ext) {
		case 'jpg':
			...
			// WEBP画像の作成
			$webp_path = preg_replace('/\.(jpg|jpeg|png|gif)$/', '.webp', $new_path);
			imagewebp($image_p, $webp_path, 80);
			break;
		case 'jpeg':
			$image = ImageCreateFromJpeg($filename);
			imagecopyresampled($image_p, $image, 0, 0, 0, 0, $new_width, $new_height, $width, $height);
...
class FileUploadBatch
{
	...
	private $photo_db_root_dir;					// 写真管理システムのルートディレクトリ
	...
	function __construct($fln,$cf, $thdir, $flw, $fname, $cre, $wcredit)
	{
		...
		$this->photo_db_root_dir = dirname(__FILE__);
		...
		for ($i = 0 ; $i < $szmax ; $i++)
		{
			if((int)$i == (int)($szmax - 1) || (int)$i == (int)($szmax - 2))
			{
				...
				if ($type == IMAGETYPE_JPEG)
				{
					...
					// WEBP画像を作成
					$webpPath = pathinfo($tmp2, PATHINFO_DIRNAME) . '/' . pathinfo($tmp2, PATHINFO_FILENAME) . '.webp';
					@imagewebp($thumb, $webpPath, 100);
				...
				if ($type == IMAGETYPE_JPEG)
				{
					...
					// WEBP画像を作成
					$webpPath = pathinfo($thfilename, PATHINFO_DIRNAME) . '/' . pathinfo($thfilename, PATHINFO_FILENAME) . '.webp';
					@imagewebp($thumb, $webpPath, 100);
