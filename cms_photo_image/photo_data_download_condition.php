...
function export_csv()
{
    global $db_link, $csv_dir, $data, $end, $init_flag, $csv_file_name, $bud_photo_no, $permit_date_from, $permit_date_to;
    
    $bud_photo_no = $_POST['bud_photo_no'];
    $permit_date_from = $_POST['permit_date_from'];
    $permit_date_to = $_POST['permit_date_to'];
    
    $images = array();
    
    if ($db_link) {
        $sql = "select * from photoimg where publishing_situation_id = 2";
        if (! empty($bud_photo_no)) {
            $sql .= " and bud_photo_no like '%" . $bud_photo_no . "%' ";
        }
        if (! empty($permit_date_from)) {
            $sql .= " and permission_date >= '" . $permit_date_from . "' ";
        }
        if (! empty($permit_date_to)) {
            $sql .= " and permission_date <= '" . $permit_date_to . " 23:59:59' ";
        }
				...
				<div class="photo_pickup">
					...
					<div class="pickup_contents">
						...
						<div style="margin-top: 20px;">
							<span style="margin-right: 10px;">BUD PHOTO番号</span> <span> <input
								type="text" id="bud_photo_no" name="bud_photo_no"
								value="<?php echo $bud_photo_no; ?>" />
								
							</span>
						</div>
						<div style="margin-top: 20px;">
							<span style="margin-right: 10px;">許可日</span> <span> <input
								type="text" id="permit_date_from" name="permit_date_from"
								readonly="readonly" value="<?php echo $permit_date_from; ?>"
								onclick="WdatePicker({ dateFmt: 'yyyy-MM-dd', maxDate: '#F{$dp.$D(permit_date_to,{d:0});}', oncleared: function () { $(this).trigger('change') }, onpicked: function () { $(this).trigger('change') } });" />
								<label style="margin: auto 5px;">～</label> <input type="text"
								id="permit_date_to" name="permit_date_to" readonly="readonly"
								value="<?php echo $permit_date_to; ?>"
								onclick="WdatePicker({ dateFmt: 'yyyy-MM-dd', minDate: '#F{$dp.$D(permit_date_from,{d:0});}', oncleared: function () { $(this).trigger('change') }, onpicked: function () { $(this).trigger('change') } });" />
