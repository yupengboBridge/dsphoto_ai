				...
				if ($pw_md5 == $user['password'])
				{
					...
					if($err != true)
					{
						...
						if ($result == true)
						{
							...
							// セッションにログイン情報を保存します。
							// xu add it on 11/29 start
							if(isset($_REQUEST['cookie_flag'])&&$_REQUEST['cookie_flag']='yes')
							{
								setcookie('login_id', $user['login_id'], time()+86400*7);
								setcookie('user_name', $user['user_name'], time()+86400*7);
								setcookie('security_level', $user['security_level'], time()+86400*7);
								setcookie('compcode', $user['compcode'], time()+86400*7);
								setcookie('group', $user['group'], time()+86400*7);
								setcookie('user_id', $user['user_id'], time()+86400*7);
							}
							// xu add it on 11/29 end
...
<div id="zentai">
	...
	<div id="contents">
		...
		<table border="0" cellspacing="0" cellpadding="0" id="login">
			...
			<tr>
				<td class="btn"><label>
					...
					<label>
					<input type="checkbox" name="cookie_flag"  value="yes" style="width:30px;"/>
					<font color="red">ログイン状態保存</font>
					</label>
				</td>
			</tr>
		</table>
		<div align="center"><!--<font color="red">
			【お知らせ】<br>
			4/4（月）19:00〜4/6（水）終日の間、<br>システムメンテナンスのため使用できません。<br>予めご了承願います。</font>-->
		</div>
		<?php if ($err == true) { ?>
			<br />
	...
	<!-- メインコンテンツ　ここまで -->
	<!--フッダーの構造は、ここから-->
	<p id="copyright">Copyright &copy; 2011 BUD International All rights reserved.</p>
