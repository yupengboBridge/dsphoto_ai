...
//liukeyu add strat 20110728
					...
					//yupengbo modify end 20120203
						...
						if(strlen($fileName)>0)
						{
							...
							mkdirs($path_dir,0777,true);
							$file_dir = $path_dir.$fileName;
							$file = fopen($file_dir,"w");//打开文件准备写入
        					fwrite($file,$img1['image1']);//写入
        					fclose($file);//关闭

       //  					$size2 = filesize($file_dir);
						...
						if(IS_WEBP == '1'){
							$l_ext = strrchr($img['photo_filename_th2'],'.');
							if ($l_ext == '.jpg'){
								...
								if(file_exists($l_webp_file_path)){
									...
									$img_webp = imagecreatefromjpeg($file);
