...
$user_need_id = array_get_value($_POST,"user_login_id","");
...
try
{
	if ($p_action == "insert")
	{
		...
		switch (array_get_value($_SESSION,"acc_sp_auth","")) {
			case 'あり':
				$sql = "UPDATE user set show_sp = '1' where login_id = '{$user_need_id}'";
				$stmt = $db_link->prepare($sql);
				$result = $stmt->execute();
				break;
			case 'なし':
				$sql = "UPDATE user set show_sp = '0' where login_id = '{$user_need_id}'";
				$stmt = $db_link->prepare($sql);
				$result = $stmt->execute();
				break;
		}
