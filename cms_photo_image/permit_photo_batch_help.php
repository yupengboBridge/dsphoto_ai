<?php
	...
	function permit_batch($photo_id)
	{
		...
		//update not to change photo_mno
		$is_update = false;
		...
		if($is_update)
		{
		    $pi->photo_mno = $s_photo_mno;
		    $flg = "ok";
		}
		else
		{
			$a = 99999 - $p_maxno;
			file_put_contents('/home/xhankyu/public_html/cms_photo_image/write_test.txt', 
			'else=='.$a.PHP_EOL, FILE_APPEND);
    		for($i=0;$i<=999999 - $p_maxno;$i++)
    		{
    			$exitstedflg1 = check_Photomon($db_link,$pi->photo_mno);
    			file_put_contents('/home/xhankyu/public_html/cms_photo_image/write_test.txt', 
			'for'.PHP_EOL, FILE_APPEND);
    			$tmp_photo_mno3 = $p_maxno+$i;
    			$pi->photo_mno = sprintf("%s-%s-%05d%s", $tmp_photo_mno1, $tmp_photo_mno2, $tmp_photo_mno3, $tmp_photo_mno4);
    			//チェック
    			$exitstedflg1 = check_Photomon($db_link,$pi->photo_mno);
    			file_put_contents('/home/xhankyu/public_html/cms_photo_image/write_test.txt', 
			$tmp_photo_mno3.PHP_EOL, FILE_APPEND);
