...
try {
    ...
    $sql = "select pi.dfrom, pi.dto, pd.image1, pi.photo_filename_th1"
            ...
            if ($now >= $img['dfrom'] && $now <= $img['dto']) {
                ...
                if (isset($_REQUEST['x']) && isset($_REQUEST['y']) && is_numeric($_REQUEST['x']) && is_numeric($_REQUEST['y']) && (int) $_REQUEST['x'] > 0 && (int) $_REQUEST['y'] > 0) {
                    ...
                    $webp = strpos($_SERVER['HTTP_ACCEPT'], 'image/webp');
                        ...
                        if(IS_WEBP == '1'){
                            $l_ext = strrchr($img['photo_filename_th1'],'.');
                            if ($l_ext == '.jpg'){
                                $l_webp_path = str_ireplace('.jpg', '.webp', $img['photo_filename_th1']);
                                # jpg文件路径
                                $l_jpg_file_path = '../'.explode($_SERVER['SERVER_NAME'], $img['photo_filename_th1'])[1];
                                # webp文件路径
