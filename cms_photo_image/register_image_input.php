...
function uploadfile()
{
	...
	try
	{
		...
		// ファイルをアップロードします。
		$fl->upload();
		//xu add it on 2010-12-27 start
		$resize_flg=true;
		//「バナー」を選択した場合に、①画像はResizeをしなくて、元のサイズのまま登録。②クレジットもしないようにする
		if($reg_div==3)
		{
			$write_credit = array(false, false, false, false, false, false);
			$fl->set_write_ok($write_credit);
			$resize_flg=false;
		}
		//xu add it on 2010-12-27 end
		// サムネイルを作成します。
		...
		pc_sp($pi->image_size_x,$pi->image_size_y,$pi->up_url);
...
function set_insertdata()
{
	...
	{
		$pi->photo_org_no = array_get_value($_POST,"photo_org_no" ,"");						// 元画像番号
		$_SESSION['photo_org_no'] = $pi->photo_org_no;										// 元画像番号
	}
	if($pi->registration_division_id==4)
	{
		$pi->photo_url = array_get_value($_POST,"photo_url" ,"");						// 元画像番号
		$_SESSION['photo_url'] = $pi->photo_url;											// 元画像番号
	}
...
function registration_division($c_id, $c_name)
{
	...
	for ($i = 0 ; $i<$ed ; $i++)
	{
		...
		if ($initflg != 1 && (int)$s_r_d_id == (int)$c_id[$i])
		{
			...
			print "	<label><input ".$onclick_function."  name=\"reg_division\" id=\"".$key_id."\" type='radio' checked=\"checked\" value=\"".$c_id[$i]."\" />".dp($c_name[$i])."</label>";
		} else {
			if($c_id[$i]=='3')
			{
				$onclick_function = " onclick='show_adv();' ";
			...
			print "	<label><input".$onclick_function." name=\"reg_division\" id=\"".$key_id."\" type='radio' value=\"".$c_id[$i]."\" />".dp($c_name[$i])."</label>";
		}
	...
	for ($i=0;$i < $ed;$i++)
	{
		...
		if ((int)$b_head_id[$i] == 2)
		{
			...
			if ($initflg != 1 && (int)$s_b_a_id == (int)$b_head_id[$i])
			{
				...
				print"		<input name=\"reg_p_obtaining_txt\" type=\"text\" id=\"reg_p_obtaining_txt\" value=".htmlspecialchars($s_r_p_o_txt,ENT_QUOTES,UTF8)." size=\"30\" />\r\n";
...
function check_input_value()
{
	...
	if (reg_divisions)
	{
		if (reg_divisions[0].checked == false && reg_divisions[1].checked == false&& reg_divisions[2].checked == false&& reg_divisions[3].checked == false)
		{
			alert('登録区分を選択してください。\r\n');
...
function init()
{
	...
	if(obj_frame) <?php 
			...
			{echo "obj_frame.style.height = '2630px';";}
			else
			{echo "obj_frame.style.height = '2500px';";}
		?>
...
<div id="contents">
	<div id="registration">
		...
		<div class="reg_file_subject">
			...
			<dl class="reg_subject reg_clear required">
				<dt>被写体の名称・バナー、ロゴの名称</dt>
				<?php  if ($initflg != 1){ ?>
		...
		<dl id="div_show_url" class="reg_division reg_clear" style="<?php $s_r_d_id = array_get_value($_SESSION,'registration_division_id' ,""); if($s_r_d_id==4&&$initflg != 1){echo "display:block;";}else{echo "display:none;";}?>">
			<dt>ページURL</dt>
			<?php  if($initflg != 1){ ?>
				<dd><textarea name="photo_url" id="photo_url" cols="70" rows="5"><?php  echo array_get_value($_SESSION,'photo_url' ,""); ?></textarea></dd>
			<?php  }else{ ?>
				<dd><textarea name="photo_url" id="photo_url" cols="70" rows="5"></textarea></dd>
			<?php  } ?>
		</dl>
		<dl id="div_show_adv" class="reg_division reg_clear" style="<?php $s_r_d_id = array_get_value($_SESSION,'registration_division_id' ,""); if($s_r_d_id==3&&$initflg != 1){echo "display:block;";}else{echo "display:none;";}?>">
			<dt>元画像番号</dt>
			<?php  if($initflg != 1){ ?>
				<dd><textarea name="photo_org_no" id="photo_org_no" cols="70" rows="5"><?php  echo array_get_value($_SESSION,'photo_org_no' ,""); ?></textarea></dd>
			<?php  }else{ ?>
		...
		<div>
			...
			<dl class="reg_addition reg_clear required">
				<dt>付加条件</dt>
				<dd>
					<?php  if($initflg != 1){ ?>
...
<!--added by wangtongchao 2011-11-26 begin-->
				<dd class="outside">
					<?php  if($initflg != 1){ ?>
						...
						?>
							<label><input name="reg_addition" id="reg_addition0" type="radio" checked="checked" value="0" onclick='change_reg_addition(this);'/>要クレジット </label>
							<input name="reg_addition_txt" type="text" id="reg_addition_txt0" value="<?php  echo  $reg_addions[0];?>" size="30"  />
							<input name="reg_addition_txt" type="text" id="reg_addition_txt1" value="<?php  echo  $reg_addions[1];?>" size="30"  />
						<?php  }else{ ?>
							<label><input name="reg_addition" id="reg_addition0" type="radio" value="0" onclick='change_reg_addition(this);'/>要クレジット </label>
							<input name="reg_addition_txt" type="text" id="reg_addition_txt0" size="30"  disabled="disabled"/>
...
<!--change by wangtongchao 2011-11-26 end reg_addition_txt1 to reg_addition_txt2-->
		...
		<div>
			...
			<dl class="reg_bud_number reg_clear required">
				<dt>BUD_PHOTO番号</dt>
				<dd>
