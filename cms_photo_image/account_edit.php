...
function form_clear()
{
	...
	if(objs)
	{
		...
		if(init_val_sec == 1)
		{
			...
			objs[4].checked = false;
		} else if(init_val_sec == 5) {
			objs[0].checked = false;
			objs[1].checked = true;
			objs[2].checked = false;
			objs[3].checked = false;
		...
		//xu modified it on 20110131 end
		
	}
	
	//yupengbo add 20110105 start
	var account_email = document.getElementById("account_email");
	var account_email_hid = document.getElementById("account_email_hid");
	if(account_email && account_email_hid) account_email.value = account_email_hid.value;
		...
		<div class="photo_pickup">
			...
			<table border="0" cellspacing="0" cellpadding="0" class="account_form">
				...
				<tr>
					...
					<td><ul>
							<li>
								...
								<input name="account_search" type="radio" id="reg_situation0" value="1" checked="checked"/>
								<?php } else { ?>
								<input name="account_search" type="radio" id="reg_situation0" value="1" />
								<?php } ?>
								：検索</label>
							</li>
							<li>
								<label>
								<?php if($usr->user_security_level == 5){ ?>
								<input name="account_search" id="reg_situation4" type="radio" value="5" checked="checked"/>
								<?php } else { ?>
								<input name="account_search" id="reg_situation4" type="radio" value="5" />
								<?php } ?>
								：検索+お客様情報検索</label>
							</li>
							<li>
								...
								<input name="account_search" id="reg_situation1" type="radio" value="2" checked="checked"/>
								<?php } else { ?>
								<input name="account_search" id="reg_situation1" type="radio" value="2" />
								<?php } ?>
								：検索・登録申請</label>
							</li>
							<li>
								...
								<input name="account_search" id="reg_situation2" type="radio" value="3" checked="checked"/>
								<?php } else { ?>
								<input name="account_search" id="reg_situation2" type="radio" value="3" />
								<?php } ?>
								：検索・登録申請・登録許可</label>
							</li>
							<li>
								...
								<input name="account_search" id="reg_situation3" type="radio" value="4" checked="checked"/>
								<?php } else { ?>
								<input name="account_search" id="reg_situation3" type="radio" value="4" />
								<?php } ?>
				...
				<tr>
					<th>メールアドレス</th>
					<td><?php 
							echo "<input type=\"hidden\" name=\"account_email_hid\" id=\"account_email_hid\" value=\"".$usr->user_email."\">\r\n";
							echo "<input type=\"text\" name=\"account_email\" id=\"account_email\" value=\"".$usr->user_email."\">\r\n";
						?>
					</td>
				</tr>
				<!-- xu add it on 2010-12-03 end -->
